...
def fastpc2oct(pc, level): 
    ...
    for index, id in enumerate(cube_ids):
        tmp_voxel =  pc_voxel[max(id[0]-fa_halflen + 1, 0):min(id[0]+fa_halflen + 1, cube_number), max(id[1]-fa_halflen + 1, 0):min(id[1]+fa_halflen + 1, cube_number), max(id[2]-fa_halflen + 1, 0):min(id[2]+fa_halflen + 1, cube_number), :]
        fa_voxel[index, :, :, :, :] = np.pad(tmp_voxel, ((max(fa_halflen-id[0]-1, 0), max(id[0]+fa_halflen+1-cube_number, 0)), (max(fa_halflen-id[1]-1, 0), max(id[1]+fa_halflen+1-cube_number, 0)), (max(fa_halflen-id[2]-1, 0), max(id[2]+fa_halflen+1-cube_number, 0)), (0, 0)), 'constant')
    print("pcvoxel", pc_voxel.shape, fa_voxel.shape)

    len_ptoc = fa_len * 2
    ptoc_voxel = np.zeros((cube_ids.shape[0], len_ptoc, len_ptoc, len_ptoc, 1), dtype=np.int8)
    ptoc_voxel[:, ::2, ::2, ::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel[:, 1::2, ::2, ::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel[:, ::2, 1::2, ::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel[:, ::2, ::2, 1::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel[:, 1::2, 1::2, ::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel[:, 1::2, ::2, 1::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel[:, ::2, 1::2, 1::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel[:, 1::2, 1::2, 1::2, :] = fa_voxel[:, :, :, :, :]
    ptoc_voxel = ptoc_voxel.reshape(cube_ids.shape[0], 1, len_ptoc, len_ptoc, len_ptoc, 1).repeat(8, axis=1)
    print("ptoc_voxel:", ptoc_voxel.shape)
    ...
    print("num of point:", n, "gtocc:", gtocc.shape)
    ...
    return octpc, query, ptoc_voxel, gtocc
