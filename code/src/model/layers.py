...
class CResBatchnormBlock(nn.Module):
    def __init__(self, global_c_dim, local_c_dim,  size_in, size_h=None, size_out=None,
                 norm_method='batch_norm', legacy=False) -> None:
        super().__init__()
         # Attributes
        if size_h is None:
            size_h = size_in
        ...
        self.global_cblk =  CResnetBlockConv1d(global_c_dim, size_in, legacy=legacy)
        self.local_cblk  =  CResnetBlockConv1d(local_c_dim, size_in, legacy=legacy)

    def forward(self, x, global_c, local_c):
        net = self.global_cblk(x, global_c)
        dx = self.local_cblk(net, local_c) 
        return x + dx
